...
then
  ...
  then
    ...
    [[ "${SO_ID}" == "centos" ]] && sudo ${PKG_MANAGER} -y install -q open-vm-tools open-vm-tools-desktop perl python2-pip
    [[ "${SO_ID}" == "ubuntu" ]] && sudo apt-get -y install -qq --no-install-recommends open-vm-tools open-vm-tools-desktop perl python2-pip
